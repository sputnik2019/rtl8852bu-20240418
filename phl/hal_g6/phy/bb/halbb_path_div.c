...
void halbb_update_tx_path_div(struct bb_info *bb, struct rtw_phl_stainfo_t *sta)
{
	...
	if ((bb->ic_type != BB_RTL8852C) && (bb->ic_type != BB_RTL8852B) && (bb->ic_type != BB_RTL8922A)) {
		BB_DBG(bb, DBG_PATH_DIV, "[%s], Early return due to wrong ic type (not 52c or 52b or 22a)\n",  __func__);
		return;
	}

	if (bb->ic_type == BB_RTL8852B){
		if ((bb->ic_sub_type != BB_IC_SUB_TYPE_8852B_8852BP) || (bb->ic_sub_type != BB_IC_SUB_TYPE_8852B_8852BPT)) {
			BB_DBG(bb, DBG_PATH_DIV, "[%s], Early return due to wrong sub-ic type (not 52bp or 52bpt)\n",  __func__);
			return;
		}
	}
	...
	if (sta->chandef.band == BAND_ON_6G) {
		...
		halbb_set_tx_path(bb, (u8)sta->macid, path);
	} else {
