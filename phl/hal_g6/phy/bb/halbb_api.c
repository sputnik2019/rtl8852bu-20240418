			...
			char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "ch_bw {ch_seg0} {ch_seg1} {band} {pri_ch} {bw} {phy_idx}\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "rfe dump\n");
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "rfe all {rfe_idx}\n");
		...
		halbb_ctrl_bw_ch(bb, (u8)val[3], (u8)val[0], (u8)val[1],
				  (enum band_type)val[2], (enum channel_width)val[4],
