	...
	const char *file_name)
{
	...
	char *sp, *ext = NULL;
	u32 para_size = 0, postfix_size = 0, i = 0, dot_pos = 0;
	u8 *para_buf = NULL;
...
read_specific_path:
	...
	if (para_size != 0) {
		/* Parsing file content */
		para_info->para_data_len = parser_fun(hal_info, para_info, para_buf,
						      para_size);
...
enum rtw_hal_status
_phl_pwrlmt_para_alloc(struct rtw_phl_com_t* phl_com, enum band_type band,
				struct rtw_para_pwrlmt_info_t *para_info)
{
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
...
#ifdef PHL_DYNAMIC_ALLOC_EXT_REGD_NAME
		if (band == BAND_ON_6G) {
			para_info->ext_regd_name = _os_mem_alloc(drv
				, sizeof(struct ext_regd_6g) * regd_name_max_size_6g);
		} else {
			para_info->ext_regd_name = _os_mem_alloc(drv
				, sizeof(struct ext_regd) * regd_name_max_size);
		}
...
#ifdef PHL_DYNAMIC_ALLOC_EXT_REGD_NAME
			if (para_info->band == BAND_ON_6G) {
				_os_mem_free(drv, para_info->ext_regd_name
					, sizeof(struct ext_regd_6g) * regd_name_max_size_6g);
			} else {
				_os_mem_free(drv, para_info->ext_regd_name
					, sizeof(struct ext_regd) * regd_name_max_size);
							...
							char *ic_name, void *hal)
{
	...
	for (idx = 0; idx < max_phy_num; idx++) {
		...
		if (_phl_pwrlmt_para_alloc(phl_com, BAND_ON_24G/* 2G and 5G use same array */,
		...
		if (_phl_pwrlmt_para_alloc(phl_com, BAND_ON_6G,
