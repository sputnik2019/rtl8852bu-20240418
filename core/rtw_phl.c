...
static void phl_msg_hub_power_mgnt_evt_hdlr(struct dvobj_priv *dvobj, u16 evt_id, u8 *buf, u32 len)
{
	_adapter *adapter = dvobj_get_primary_adapter(dvobj);
	struct wiphy *wiphy = adapter_to_wiphy(adapter);
	enum rtw_rf_state *new_rf_state = NULL;
	static enum rtw_rf_state ori_rf_state = RTW_RF_ON;

	PHL_INFO("%s : evt_id %d.\n", __func__, evt_id);

	if (buf == NULL) {
		PHL_ERR("%s : Msg info buffer is NULL!\n", __func__);
		return;
	}

	if (len != sizeof(*new_rf_state)) {
		PHL_ERR("%s : Msg info len does not match!\n", __func__);
		return;
	}
	...
	case MSG_EVT_HW_RF_CHG:
		if (ori_rf_state != *new_rf_state) {
			wiphy_rfkill_set_hw_state(wiphy, (*new_rf_state == RTW_RF_OFF ? 1 : 0));
...
void core_handler_phl_msg(void *drv_priv, struct phl_msg *msg)
{
	...
	case PHL_MDL_POWER_MGNT:
		phl_msg_hub_power_mgnt_evt_hdlr(dvobj, evt_id, msg->inbuf, msg->inlen);
		break;
	default:
...
static s8 phy_get_txpwr_user_lmt(struct dvobj_priv *dvobj, u8 ntx_idx)
{
	...
	if (total_mbm != UNSPECIFIED_MBM)
		lmt = (total_mbm - mb_of_ntx(ntx_idx + 1)) * rtw_phl_get_tx_tbl_to_tx_pwr_times(phl_info) / MBM_PDBM;
	else
...
void rtw_txpwr_hal_update_pwr(struct dvobj_priv *dvobj, enum phl_band_idx band_idx)
{
	...
	s8 lmt_na = rtw_phl_get_power_limit_value_na(phl_info);
	int i;

	/* ext_pwr_lmt is for all hwband here */
	rtw_set_ext_pwr_lmt_into(dvobj, &ext_pwr_lmt_info);
	if (phy_get_txpwr_user_lmt(dvobj, 0) != lmt_na) {
